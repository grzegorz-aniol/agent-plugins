...
| `double-check` | Skill `double-check` — a post-implementation self-review. When the agent reports a task as done, this skill makes it re-read everything it changed in the session, hunting for bugs, inconsistencies, overcomplicated code, oversized functions, and YAGNI/KISS/DRY violations. Invoke it manually (`/double-check`) or let the agent pick it up when it finishes an implementation. |
